...
int solve(int ind, vector<int> &heights, vector<int> &cache)
{
    ...
    int oneJump = abs(heights[ind] - heights[ind - 1]) + solve(ind - 1, heights, cache);

    int twoJumps = INT_MAX;
    if (ind > 1)
    {
        twoJumps = abs(heights[ind] - heights[ind - 2]) + solve(ind - 2, heights, cache);
    }

    cache[ind] = min(oneJump, twoJumps);
