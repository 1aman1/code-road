...
void Heap::Heapify(int index, int heapLen)
{
    ...
    if (leftChild < heapLen && elements[leftChild] > elements[largest])
        largest = leftChild;

    if (rightChild < heapLen && elements[rightChild] > elements[largest])
        largest = rightChild;
...
bool Heap::Is_Heap()
{
    for (auto i = 0; i < Container::size / 2; ++i)
    {
        ...
        if ((elements[leftChild] > elements[i]) ||
            (right < Container::size && elements[right] > elements[i]))
            return false;
    }
